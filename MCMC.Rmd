...
out <- runMCMC(bayesianSetup = setup, sampler = "SMC", settings = settings)
saveRDS(out, paste0("outputs/smc_", model_name, "_seed_", s, "_", tstamp(hour=T, minute=T), ".rds"))
...
```{r}
# fls = list.files("outputs/", "smc_sepiar_stoch_seed.*", full.names = TRUE)
fls = list.files("outputs/", "smc_sepiar_euler_seed.*", full.names = TRUE)

LL = rep(NA, length(fls))
params = matrix(NA, nrow=length(fls), ncol=4)

for (i in seq_along(fls)) {
  d = readRDS(fls[i])
  pars_sm = apply(d$particles, 2, summary)
  params[i,] = pars_sm[4,]
  # print(apply(d$particles, 2, summary))
  ll = rep(NA, 200)
  for (j in 1:200) {
    l = loglik(pars = params[i,])
    while (!is.finite(l)) {
      l = loglik(pars = params[i,])
    }
    ll[j] = l
  }
  LL[i] = mean(ll, na.rm=TRUE)
}
params[which.max(LL),]
LL[which.max(LL)]
# from MCMC
# parm = c(34.69508, 8.279553, 2.724505, 0.5776921)
# from SMC
parm = c(30.86321, 9.693134, 2.549154, 0.5717748)
...
upper=c(100,20 ,20, 1) 
set.seed(3)
outDEoptim <- DEoptim(fn=negloglik, lower=lower, upper=upper)

outDEoptim <- DEoptim(fn=negloglik, lower=lower, upper=upper, control=DEoptim.control(NP=200, itermax=4000))

parm = c(34.855471, 8.292033, 2.719610, 0.573286)
parm = c(34.641208, 8.289339, 2.778588, 0.574176)
# PARAMETERS$model <- sepiar_euler
PARAMETERS$model <- sepiar_stoch
loglik(pars = parm)
...
```{r}
boot_inc <- function(dat, n=100) {
  bootsamp = matrix(NA, nrow=length(dat), ncol=n)
  for(i in seq_len(n)){
    set.seed(i)
    bootsamp[,i] = rpois(length(dat), dat)
  }
  return(bootsamp)
}
PARAMETERS$model <- sepiar_euler
samp = boot_inc(dat=OBS,n=200)
pars = matrix(NA, nrow=ncol(samp), ncol=5)
for (i in 1:ncol(samp)) {
  OBS = samp[,i]
  out = DEoptim(fn=negloglik, lower=lower, upper=upper,
                control=DEoptim.control(itermax=200, trace=TRUE))
  pars[i,1:4] = out$optim$bestmem
  pars[i,5] = out$optim$bestval
}
apply(pars, 2, quantile, probs=c(0.025, 0.5,0.975))
pars_ode = pars
# 2.5%  34.50609 8.078429 2.683293 0.5744306 192304.1
# 50%   35.14728 8.289496 2.745928 0.5753268 192735.2
# 97.5% 36.26695 8.540605 2.789779 0.5781287 193316.9
PARAMETERS$model <- sepiar_stoch
pars = matrix(NA, nrow=ncol(samp), ncol=5)
for (i in 1:ncol(samp)) {
  OBS = samp[,i]
  out = DEoptim(fn=negloglik, lower=lower, upper=upper,
                control=DEoptim.control(itermax=1000, trace=TRUE))
  pars[i,1:4] = out$optim$bestmem
  pars[i,5] = out$optim$bestval
}
apply(pars, 2, quantile, probs=c(0.025, 0.5,0.975))
pars_stoch = pars
# 2.5%  32.69280 8.256353 2.608057 0.5704892 190626.9
# 50%   34.27013 8.400966 2.707926 0.5769147 191274.0
# 97.5% 36.68250 8.634674 2.763187 0.5798849 191753.1


OBS <- c(unreported, dat$symptomatic[1:PARAMETERS$obslength])  
demin = findmin(iter=30)
...
fit <- readRDS("outputs/mcmc_sepiar_euler_20230329T1935.rds")
nrun <- 200
npar <- 4
(nr = length(fit[[1]]$chain[,1]))
allpars = lapply(fit, function(x) x$chain[round(nr/2):nr,1:npar])
allpars = do.call('rbind', allpars)

# (nr=nrow(fit$particles))
set.seed(42)
pars <- allpars[sample(1:nrow(allpars), nrun),]
# pars <- chain[sample(round(nr/2):nr, nrun),1:4]
...
p
ggsave(sprintf("plots/mcmc_fit_%s.png", tstamp()), p, width=3.4*2, height=2.7*2)
p <- plot_model_data(model=sim, data=dat, var="inf")
p
ggsave(sprintf("plots/mcmc_fit_inf_symp_%s.png", tstamp()), p, width=3.4*2, height=2.7*2)
```
...
create_summary_table = function(dat,sim,pars,probs=c(0.025, 0.5, 0.975),cfr=0.003) {
  ...
  date_intro_stat = dat$date[1] - rev(sim_date_intro) # rev to rearrange from diff to dates 
