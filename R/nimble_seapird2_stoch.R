nimble_seapird2_stoch <- nimbleFunction(
  ...
  tau <- 1 # time step size in terms of a day
## -----------------------------------------------------------------
  ...
  #   new_infections = rpois(1, St * foi * tau)
  #   EtoP = rpois(1, Et * epsilon * (1-fA) * tau)
  #   EtoA = rpois(1, Et * epsilon * fA * tau)
  #   new_symptoms = rpois(1, Pt * rate_P_I * tau)
  #   ItoR = rpois(1, It * (1-cfr) * gamma * tau)
  #   AtoR = rpois(1, At * 1 / (1/gamma + 1/rate_P_I) * tau)
  #   new_deaths = rpois(1, It * cfr * eta * tau)
...
## -----------------------------------------------------------------
  ...
  for (day in 2:ndays) {
    for(step in 1:stepsperday) {# // Essentially Euler method was implemented
      ...
      EtoA = 0
      if (Et > 0) {
        fromE = rmulti(1, Et, c(epsilon * (1-fA) * tau, epsilon * fA * tau))
        EtoP = fromE[1]
        EtoA = fromE[2]
      }
      new_symptoms = 0
